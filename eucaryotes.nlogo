...
globals [colour i_low i_up index fuite]
...
;;Procedure to move the amibes :
to amibe-movement
  ifelse (random 10) <= (5 * (c + 0.5))      ;; proba d'aller dans la bonne direction => dépend de la concentration de la case où on veut aller
    [mouvement-aleatoire]
    [chemotaxis_amibe]
    set c (c - (conso-c-amibes * c))

end

;;Procedure to move amibes when they are attracted :
to chemotaxis_amibe
  ask amibes [   ;; tortues attiré par attractant à mettre dans go
    let c_temp 0
    ask neighbors [ set c_temp (c_temp + c)]  ;; détection d'absence d'espèce ou pas
    ifelse c_temp = 0
    [mouvement-aleatoire]  ;; absence d'espèce
    [ let c_max [0]
      let pxmax [0]      ;; création des listes temporaires accueillant la c° et
      let pymax [0]      ;;      les coordonnées des cases avec le plus d'espèce
      let k 0            ;; index temporaire
      ask neighbors with [not any? amibes-here] [
        if c > item 0 c_max [
            set c_max replace-item 0 c_max c
            set pxmax replace-item 0 pxmax pxcor   ;; remplacement par valeur supérieure
            set pymax replace-item 0 pymax pycor
        ]
        if c = item 0 c_max [
          set k (k + 1)
          set c_max insert-item k c_max c
          set pxmax insert-item k pxmax pxcor     ;; ajout de plusieurs cases max c° d'espèce
          set pymax insert-item k pymax pycor
        ]
      ]
      let i_temp random ((length c_max));; gérer le cas où après avoir eu deux cases de suite avec la même valeur max
      foreach c_max [ [iii] ->          ;; je trouve une nouvelle valeur max après => je prend la première valeur max sans être aléatoire
        if (iii) != item 0 c_max [set i_temp 0]  ;; iii= élément de la liste
      ]
      ifelse (item i_temp c_max - c) >= 0.001   ;; ici ça sert à faire en sorte que notre gonze bouge que si il y a une différence de concentration suffisante
      [
      if c_max != [0] [ ;;merde à quoi ça sert ce if ?
        setxy (item i_temp pxmax) (item i_temp pymax)   ;; mouvement
      ]]
        [mouvement-aleatoire]
...
;;Procedure to move amibes when they're not attracted :
to mouvement-aleatoire
  set heading random 360     ;; mouvement aléatoire
...
;;Command to create agent on the coordinates specified by the mouse
to put-turtle-mouse
  if mouse-down?
    [reset-ticks
     clear-plot
...
This model reproduce the chemotactic behavior of dicty.

## HOW IT WORKS

We make a chemoattractant that diffuse in space.

Dicty agent spread randomly and in presence of the chemoattractant they biased their movement toward it.

## HOW TO USE IT

Diffuse an attractant, and put some agents that represents dicty.

## THINGS TO TRY/NOTICE

Try to put different sources to observe a local/global problem. Make dicty consume the chemoattractant to reverse this problem.

## EXTENDING THE MODEL

Try to optimize the model to run it faster.
...
true
0
Circle -11221820 true false 116 101 67
Circle -11221820 true false 120 135 0
Circle -11221820 true false 150 135 30
Circle -11221820 true false 165 135 30
Circle -11221820 true false 165 105 30
Circle -11221820 true false 150 90 30
Circle -11221820 true false 165 120 30
Circle -11221820 true false 150 105 30
Circle -11221820 true false 150 90 30
Circle -11221820 true false 120 90 30
Circle -11221820 true false 135 150 30
Circle -11221820 true false 105 120 30
Circle -11221820 true false 120 135 30
Circle -11221820 true false 105 135 30
Circle -11221820 true false 135 90 30
...
Circle -16777216 true false 135 75 60
Circle -16777216 true false 195 75 60
Polygon -16777216 true false 165 240 105 224 77 198 62 176 82 164 105 188 124 203 165 210 207 203 225 188 242 166 266 179 251 202 227 225
Polygon -11221820 true false 15 60 285 60
Polygon -1 true false 15 75 285 75 270 30 270 -30 30 -30 30 30 15 75
Polygon -2674135 true false 165 45 150 75 195 75 180 45 210 60 210 15 180 30 195 0 150 0 165 30 135 15 135 60 165 45
Polygon -2674135 true false 60 90
Polygon -2674135 true false 45 285 45 90 0 105 0 45 45 60 30 15 90 15 75 60 120 45 120 105 75 90 75 285 45 285
